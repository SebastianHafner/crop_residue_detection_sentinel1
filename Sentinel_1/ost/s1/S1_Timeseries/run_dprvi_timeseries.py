...
# -*- coding: utf-8 -*-
"""
Sentinel-1 SLC → DpRVI only (Mandal et al. 2020)
...
PRODUCT_KEY = "dprvi"
FINAL_PATTERN = "*dprvi_SWEREF99TM_10m.tif"
MARKER = ".dprvi.processed"
...
def process_bursts(burst_gdf, scene_id, scene_zip, out_root, config_file):
    ...
    for idx, row in burst_gdf.iterrows():
        ...
        dprvi_final = final_dir / f"{prefix}_dprvi_SWEREF99TM_10m.tif"

        # Skip if already done
        if dprvi_final.exists():
            final_tifs.append(str(dprvi_final))
        ...
        try:
            ...
            if out_dprvi and Path(out_dprvi).exists():
                print(f"  ✓ DpRVI")
                reproject_to_sweref(Path(out_dprvi), dprvi_final)
                final_tifs.append(str(dprvi_final))
                print(f"    → {dprvi_final.name}")
            elif err:
                print(f"  ⚠ {err}")
...
def merge_by_date(out_root: Path):
    ...
    into a single dated mosaic: <YYYYMMDD>_dprvi_SWEREF99TM_10m.tif
    """
    merged_dir = out_root / "merged_SWEREF99TM_10m"
    merged_dir.mkdir(exist_ok=True)

    scene_tifs = [
        t for t in sorted(merged_dir.glob("*_dprvi_SWEREF99TM_10m.tif"))
    ...
    for date, group in groupby(sorted(scene_tifs, key=get_date), key=get_date):
        ...
        out_tif = merged_dir / f"{date}_dprvi_SWEREF99TM_10m.tif"
...
def main():
    ap = argparse.ArgumentParser(description="S1 SLC → DpRVI → SWEREF99TM 10m (all zips)")
    ...
    ap.add_argument("--dprvi_window", type=int, default=3)
    ...
    for scene_zip in scene_zips:
        ...
        print("=" * 60)
        print("SENTINEL-1 SLC → DpRVI (Mandal et al. 2020)")
        print("=" * 60)
        print(f"Scene:    {scene_zip.name}")
        print(f"Product:  {args.product_type}")
        print(f"Window:   {args.dprvi_window}")
        ...
        config_dict = build_config(
            ...
            backscatter=False, haalpha=False, dprvi=True,
            dprvi_window=args.dprvi_window,
            product_type=args.product_type, resolution=args.resolution,
        )
        config_file = out_root / "config_dprvi.json"
        ...
        if not args.no_merge and final_tifs:
            merged_dir = out_root / "merged_SWEREF99TM_10m"
            merged_dir.mkdir(exist_ok=True)
            out_tif = merged_dir / f"{scene_id}_dprvi_SWEREF99TM_10m.tif"
            print(f"\n{'=' * 60}")
            print(f"MERGING {len(final_tifs)} bursts for {scene_id}")
            if merge_burst_tifs(final_tifs, out_tif):
                mb = out_tif.stat().st_size / (1024 * 1024)
                print(f"  ✓ DpRVI: {out_tif.name} ({mb:.1f} MB)")
