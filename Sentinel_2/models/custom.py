...
class PixelEncoderNet(nn.Module):

    def __init__(self, input_dim: int, mlp1: List[int] = (32, 64), pooling: str = 'mean_std',
                 mlp2: List[int] = (128, 128), mlp4: List[int] = (128, 64, 32), output_dim: int = 1):
        super().__init__()
        mlp1 = [input_dim] + mlp1
        mlp4 = mlp4 + (output_dim,)
        self.pixel_set_encoder = PixelSetEncoder(mlp1=mlp1, pooling=pooling, mlp2=mlp2)
        self.decoder = get_decoder(mlp4)
        self.name = 'pixelencodernet'

    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        """
         Args:
            x(Tensor): B x T x C x P
            mask(Tensor): B x P
        """
        out = self.pixel_set_encoder(x, mask)
        assert out.size(1) == 1
        out = out[:, 0]
...
class PixelSetEncoder(nn.Module):
    def __init__(self, mlp1: List[int] = (4, 32, 64), pooling: str = 'mean_std',
                 mlp2: List[int] = (64, 128)):
        """
        Pixel-set encoder.
        Args:
        ...
        self.input_dim = mlp1[0]
        self.mlp1_dim = copy.deepcopy(mlp1)
        self.mlp2_dim = copy.deepcopy(mlp2)
        self.pooling = pooling

        self.output_dim = self.input_dim * len(pooling.split('_')) if len(self.mlp2_dim) == 0 else self.mlp2_dim[-1]
